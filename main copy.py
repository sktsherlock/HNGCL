...
def train(feature_graph_edge_index, drop_weights1, drop_weights2):
    ...
    loss = model.loss(z1, z2, z3, batch_size=256)
...
def train_hard(feature_graph_edge_index, drop_weights1, drop_weights2, traih_warmup, train_dis):
    ...
    if train_dis:
        z4 = ADNet(x_1, edge_index_1)
        z4 = ADNet.Generate_hard(z1, z4)
        ...
        for i in range(10):
            ...
            loss_Dis = 1 - Dis(discriminator, z1, z4)
            #print(f'(loss_Dis={loss_Dis:.4f})')
    ...
    return loss.item() #, loss_Dis.item()
...
if __name__ == '__main__':
    ...
    #
    # hard_generator = Negative_Generator(param['num_hidden'], param['num_proj_hidden'], param['num_hidden'])
    # hard_optimizer = torch.optim.Adam(
    #     hard_generator.parameters(),
    ...
    # netD = define_D(opt.output_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
    # optimizer_D = torch.optim.Adam(netD.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
    
    ADNet = ADNet(dataset.num_features, param['hard_num'], get_activation(param['activation']),
                      base_model=get_base_model(param['base_model']), k=param['num_layers']).to(device)
    ...
    if param['drop_scheme'] == 'degree':
        ...
        drop_weights2 = degree_drop_weights(feature_graph_edge_index).to(device)
    elif param['drop_scheme'] == 'pr':
        drop_weights1 = pr_drop_weights(data.edge_index, aggr='sink', k=200).to(device)
        drop_weights2 = pr_drop_weights(feature_graph_edge_index, aggr='sink', k=200).to(device)
    elif param['drop_scheme'] == 'evc':
        drop_weights1 = evc_drop_weights(data).to(device)
        drop_weights2 = drop_weights1[:feature_graph_edge_index.size()[1]]
    else:
        drop_weights1 = None
        drop_weights2 = None
    ...
    # result_file = osp.join(result_file_path, args.dataset, "{}_epoches_{}NN_on_{}_result.txt".format(param['num_epochs'], param['k'], args.dataset))
    # check_dir(result_file)

    # record_hyper_parameter(result_file, param)
