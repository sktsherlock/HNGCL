...
def train(model,  feature_graph_edge_index, drop_weights1, drop_weights2, args):
    model.train()
    optimizer.zero_grad()
...
#     # loss = model.loss_neg(z1, z2, batch_size=512)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, param['num_epochs'] + 1):
        time_start = time.time()
        loss = train(model,  feature_graph_edge_index, drop_weights1, drop_weights2, args)
        #loss = train(model, data.x, data.edge_index, feature_graph_edge_index)
        time_end = time.time()
        time_c= time_end - time_start
        print('time cost', time_c, 's')
